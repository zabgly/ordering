<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="菜品ID" prop="productId">
        <el-input
    ...
    <el-table v-loading="loading" :data="linkList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="菜品ID" align="center" prop="productId" />
      <el-table-column label="促销ID" align="center" prop="promotionId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="菜品ID" prop="productId">
          <el-input v-model="form.productId" placeholder="请输入菜品ID" />
        </el-form-item>
        <el-form-item label="促销ID" prop="promotionId">
          <el-input v-model="form.promotionId" placeholder="请输入促销ID" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        promotionId: [
          { required: true, message: "促销ID不能为空", trigger: "blur" }
        ],
        createTime: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除菜品促销关联编号为"' + ids + '"的数据项？').then(function() {
        return delLink(ids);
      }).then(() => {
        this.getList();
